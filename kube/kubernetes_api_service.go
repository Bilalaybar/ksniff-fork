...
type KubernetesApiService interface {
	...
	CreatePrivilegedPod(nodeName string, containerName string, image string, socketPath string, timeout time.Duration, serviceaccount string, toleration string) (*corev1.Pod, error)
...
func (k *KubernetesApiServiceImpl) CreatePrivilegedPod(nodeName string, containerName string, image string, socketPath string, timeout time.Duration, serviceaccount string, toleration string) (*corev1.Pod, error) {
	...
	podSpecs.Tolerations = []corev1.Toleration{tolerations}
